...
router.post("/purchase", (req, res, next) => {
  ...
  const query1 =
    "INSERT INTO tblPurchase (ContractNumber, Description, Height, Width, Length, OrderQty, KgPerLength, PricePerLength) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  const query2 =
    "INSERT INTO tblStock (ContractNumber, Description, Height, Width, Length, OrderQty, KgPerLength, PricePerLength) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  const query3 =
    "INSERT INTO tbStock (PurchaseID) VALUES (SELECT ID FROM tblPurchases WHERE ";
...
router.get("/details/:description", (req, res, next) => {
  ...
  const query =
    "SELECT Height, Width, Length, KgPerLength, PricePerLength FROM tblPriceList WHERE Description = ?";
